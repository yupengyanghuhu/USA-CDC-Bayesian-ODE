...
def projection(input_dict, tsteps):
	...
	core.RunProjections.RunProjections(AcceptedUniqueParameters, tmax, fitData, countyPopulation, InitialInfections, 
		InitialExposedMult, lockdown_begin, lockdown_duration, Dates, SimLabel, RunProjections, multiprocessingma)
...
def main():
	...
	countyNames = open('USCountiesList.txt', "r").read().split('\n')
	if countyNames[-1] == '':
		countyNames.pop(-1)
	...
	for i in range(len(countyNames)):
		...
		try:
			print('*********************')
			print('******* Running '+ countyName +' (has '+ str(len(countyNames) -1 - i) +' counties left)*******')
			for root, dirs, files in os.walk("data/countiesData"):
				for fileName in files:
					if countyName in fileName:
						...
						input_dict['countyDataFile'] = 'data/countiesData/' + fileName
						...
						input_dict['multiprocessingma'] = False
			...
			countyWithProblem.append(countyName)
			pass

		with open('countyWithProblem.txt', 'w') as f:
			for c in countyWithProblem:
				f.write("%s\n" % c)
